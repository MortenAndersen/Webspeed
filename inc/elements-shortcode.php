...
/* -------------------------------------- */
	...
	if ($loop->have_posts()) {
		...
		echo '<div id="nav" class="' . $scroll_margin . '">';
			while ($loop->have_posts()): $loop->the_post();
			$id = get_the_ID();
        ...
        while ($loop->have_posts()): $loop->the_post();
			...
			echo '<div id="' . $id . '" class="' . $scroll_margin . '">';
				the_title('<h2>', '</h2>');
				the_content(  );
				echo '<p><a href="#nav"><svg style="width:20px" width="100%" height="100%" viewBox="0 0 70 70" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
